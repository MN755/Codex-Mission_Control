...
PLUGIN_MANIFEST = json.loads((ROOT / "plugins" / "mission-control" / "plugin.json").read_text(encoding="utf-8"))
PROMPTS_CATALOG = json.loads((ROOT / "plugins" / "mission-control" / "mcp" / "prompts.json").read_text(encoding="utf-8"))

MANIFEST_RESOURCES = PLUGIN_MANIFEST["resources"]
PROMPT_TITLES = [item["title"] for item in PROMPTS_CATALOG["prompts"]]
ARCHITECTURE_TOOLS = [
...
add(
    ...
    [
        ...
        (
            "Expected tools",
            "Expected tools:\n\n" + bullet_lines(ARCHITECTURE_TOOLS),
        ),
        (
            "Expected resources",
            "Expected resources:\n\n" + bullet_lines(MANIFEST_RESOURCES),
        ),
        (
...
add_reference_page(
    ...
    [f"`{resource}`" for resource in MANIFEST_RESOURCES],
