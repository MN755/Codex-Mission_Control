...
def test_bundled_manifest_mcp_paths_exist() -> None:
    manifest = json.loads((BUNDLED_ROOT / "plugin.json").read_text(encoding="utf-8"))
    bundled_paths = [
        BUNDLED_ROOT / manifest["mcp"]["example_config"].removeprefix("./"),
        BUNDLED_ROOT / manifest["mcp"]["resources_catalog"].removeprefix("./"),
        BUNDLED_ROOT / manifest["mcp"]["prompts_catalog"].removeprefix("./"),
    ]
    for path in bundled_paths:
        assert path.exists(), f"Missing bundled MCP asset: {path}"
