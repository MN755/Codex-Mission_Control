...
from models import Agent, AgentArchetype, ImportedCodebaseSafety, Project, RiskRecord, SecurityPolicy, SwarmPreferences, ValidationCoverageArea
...
def test_risk_update_route_requires_matching_project_scope(client) -> None:
    project_one = _create_project(client, "Risk Scope One", "risk-scope-one")
    project_two = _create_project(client, "Risk Scope Two", "risk-scope-two")
    ...
    correct_update = client.patch(
        f"/api/risks/{risk_id}",
        params={"project_id": project_one["id"]},
        json={"status": "mitigated"},
    )
    assert correct_update.status_code == 200, correct_update.text
    assert correct_update.json()["status"] == "mitigated"
