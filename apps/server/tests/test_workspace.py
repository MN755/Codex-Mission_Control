...
def test_legacy_project_without_settings_reads_defaults_without_persisting_row(client) -> None:
    init_db()
    db = SessionLocal()
    try:
        project = Project(
    ...
    system_status_response = client.get(f"/api/system/status?project_id={project_id}")
