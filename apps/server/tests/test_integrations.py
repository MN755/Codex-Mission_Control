...
def test_project_integrations_and_action_preview_flow(client, bridge_headers, monkeypatch, tmp_path) -> None:
    ...
    def _sum_count_maps(field_name: str) -> dict[str, int]:
        ...
        return counts
    def _group_list_family_ids(field_name: str) -> dict[str, list[str]]:
        grouped = {}
        for item in family_list:
            seen = set()
        ...
        return grouped
    def _group_count_map_family_ids(field_name: str) -> dict[str, list[str]]:
        grouped = {}
        for item in family_list:
            for key, value in item[field_name].items():
                if not value:
    ...
    def _families_with_values(field_name: str) -> list[str]:
        return [item["family"] for item in family_list if item[field_name]]
    def _families_with_count_map_values(field_name: str) -> list[str]:
        return [item["family"] for item in family_list if any(item[field_name].values())]
    def _merge_unique_strings(field_name: str) -> list[str]:
        values = []
        seen = set()
        for item in family_list:
            for value in item[field_name]:
    ...
    def _collect_action_refs(field_name: str) -> list[str]:
        refs = []
        for item in family_list:
            refs.extend(f'{item["family"]}:{action_id}' for action_id in item[field_name])
        return refs
    def _collect_grouped_action_refs(field_name: str) -> dict[str, list[str]]:
        grouped = {}
        for item in family_list:
            for key, action_ids in item[field_name].items():
                grouped.setdefault(key, []).extend(f'{item["family"]}:{action_id}' for action_id in action_ids)
        return grouped
    def _sum_int_field(field_name: str) -> int:
        return sum(item[field_name] for item in family_list)
    def _family_ids_for_positive_count(field_name: str) -> list[str]:
        return [item["family"] for item in family_list if item[field_name] > 0]
    ...
    assert project_integrations_payload["available_action_refs"] == _collect_action_refs("available_action_ids")
    assert project_integrations_payload["local_action_count"] == _sum_int_field("local_action_count")
    assert project_integrations_payload["local_action_refs"] == _collect_action_refs("local_action_ids")
    assert project_integrations_payload["local_action_family_count"] == sum(1 for item in family_list if item["local_action_count"] > 0)
    assert project_integrations_payload["local_action_family_ids"] == _family_ids_for_positive_count("local_action_count")
    assert project_integrations_payload["guided_action_count"] == _sum_int_field("guided_action_count")
    assert project_integrations_payload["guided_action_refs"] == _collect_action_refs("guided_action_ids")
    assert project_integrations_payload["guided_action_family_count"] == sum(1 for item in family_list if item["guided_action_count"] > 0)
    assert project_integrations_payload["guided_action_family_ids"] == _family_ids_for_positive_count("guided_action_count")
    assert project_integrations_payload["registry_action_count"] == _sum_int_field("registry_action_count")
    assert project_integrations_payload["registry_action_refs"] == _collect_action_refs("registry_action_ids")
    assert project_integrations_payload["registry_action_family_count"] == sum(1 for item in family_list if item["registry_action_count"] > 0)
    assert project_integrations_payload["registry_action_family_ids"] == _family_ids_for_positive_count("registry_action_count")
    assert project_integrations_payload["provider_specific_action_count"] == _sum_int_field("provider_specific_action_count")
    assert project_integrations_payload["provider_specific_action_refs"] == _collect_action_refs("provider_specific_action_ids")
    assert project_integrations_payload["provider_specific_action_family_count"] == sum(1 for item in family_list if item["provider_specific_action_count"] > 0)
    assert project_integrations_payload["provider_specific_action_family_ids"] == _family_ids_for_positive_count("provider_specific_action_count")
    assert project_integrations_payload["guided_only_action_count"] == _sum_int_field("guided_only_action_count")
    assert project_integrations_payload["guided_only_action_refs"] == _collect_action_refs("guided_only_action_ids")
    assert project_integrations_payload["guided_only_action_family_count"] == sum(1 for item in family_list if item["guided_only_action_count"] > 0)
    assert project_integrations_payload["guided_only_action_family_ids"] == _family_ids_for_positive_count("guided_only_action_count")
    assert project_integrations_payload["available_mutating_action_count"] == _sum_int_field("available_mutating_action_count")
    assert project_integrations_payload["available_mutating_action_refs"] == _collect_action_refs("available_mutating_action_ids")
    assert project_integrations_payload["available_mutating_action_family_count"] == sum(1 for item in family_list if item["available_mutating_action_count"] > 0)
    assert project_integrations_payload["available_mutating_action_family_ids"] == _family_ids_for_positive_count("available_mutating_action_count")
    assert project_integrations_payload["available_non_mutating_action_count"] == _sum_int_field("available_non_mutating_action_count")
    assert project_integrations_payload["available_non_mutating_action_refs"] == _collect_action_refs("available_non_mutating_action_ids")
    assert project_integrations_payload["available_non_mutating_action_family_count"] == sum(1 for item in family_list if item["available_non_mutating_action_count"] > 0)
    assert project_integrations_payload["available_non_mutating_action_family_ids"] == _family_ids_for_positive_count("available_non_mutating_action_count")
    ...
    assert project_integrations_payload["blocked_action_refs"] == _collect_action_refs("blocked_action_ids")
    ...
    assert project_integrations_payload["execution_action_refs"] == _collect_action_refs("execution_action_ids")
    assert project_integrations_payload["available_execution_action_count"] == sum(item["available_execution_action_count"] for item in family_list)
    assert project_integrations_payload["available_execution_action_refs"] == _collect_action_refs("available_execution_action_ids")
    assert project_integrations_payload["available_execution_family_count"] == sum(1 for item in family_list if item["available_execution_action_count"] > 0)
    assert project_integrations_payload["available_execution_family_ids"] == [item["family"] for item in family_list if item["available_execution_action_count"] > 0]
    assert project_integrations_payload["local_execution_action_count"] == _sum_int_field("local_execution_action_count")
    assert project_integrations_payload["local_execution_action_refs"] == _collect_action_refs("local_execution_action_ids")
    assert project_integrations_payload["local_execution_action_family_count"] == sum(1 for item in family_list if item["local_execution_action_count"] > 0)
    assert project_integrations_payload["local_execution_action_family_ids"] == _family_ids_for_positive_count("local_execution_action_count")
    assert project_integrations_payload["guided_execution_action_count"] == _sum_int_field("guided_execution_action_count")
    assert project_integrations_payload["guided_execution_action_refs"] == _collect_action_refs("guided_execution_action_ids")
    assert project_integrations_payload["guided_execution_action_family_count"] == sum(1 for item in family_list if item["guided_execution_action_count"] > 0)
    assert project_integrations_payload["guided_execution_action_family_ids"] == _family_ids_for_positive_count("guided_execution_action_count")
    assert project_integrations_payload["provider_specific_execution_action_count"] == _sum_int_field("provider_specific_execution_action_count")
    assert project_integrations_payload["provider_specific_execution_action_refs"] == _collect_action_refs("provider_specific_execution_action_ids")
    assert project_integrations_payload["provider_specific_execution_action_family_count"] == sum(1 for item in family_list if item["provider_specific_execution_action_count"] > 0)
    assert project_integrations_payload["provider_specific_execution_action_family_ids"] == _family_ids_for_positive_count("provider_specific_execution_action_count")
    assert project_integrations_payload["guided_only_execution_action_count"] == _sum_int_field("guided_only_execution_action_count")
    assert project_integrations_payload["guided_only_execution_action_refs"] == _collect_action_refs("guided_only_execution_action_ids")
    assert project_integrations_payload["guided_only_execution_action_family_count"] == sum(1 for item in family_list if item["guided_only_execution_action_count"] > 0)
    assert project_integrations_payload["guided_only_execution_action_family_ids"] == _family_ids_for_positive_count("guided_only_execution_action_count")
    assert project_integrations_payload["mutating_execution_action_count"] == _sum_int_field("mutating_execution_action_count")
    assert project_integrations_payload["mutating_execution_action_refs"] == _collect_action_refs("mutating_execution_action_ids")
    assert project_integrations_payload["mutating_execution_action_family_count"] == sum(1 for item in family_list if item["mutating_execution_action_count"] > 0)
    assert project_integrations_payload["mutating_execution_action_family_ids"] == _family_ids_for_positive_count("mutating_execution_action_count")
    assert project_integrations_payload["non_mutating_execution_action_count"] == _sum_int_field("non_mutating_execution_action_count")
    assert project_integrations_payload["non_mutating_execution_action_refs"] == _collect_action_refs("non_mutating_execution_action_ids")
    assert project_integrations_payload["non_mutating_execution_action_family_count"] == sum(1 for item in family_list if item["non_mutating_execution_action_count"] > 0)
    assert project_integrations_payload["non_mutating_execution_action_family_ids"] == _family_ids_for_positive_count("non_mutating_execution_action_count")
    assert project_integrations_payload["confirmation_required_execution_action_count"] == _sum_int_field("confirmation_required_execution_action_count")
    assert project_integrations_payload["confirmation_required_execution_action_refs"] == _collect_action_refs("confirmation_required_execution_action_ids")
    assert project_integrations_payload["confirmation_required_execution_action_family_count"] == sum(1 for item in family_list if item["confirmation_required_execution_action_count"] > 0)
    assert project_integrations_payload["confirmation_required_execution_action_family_ids"] == _family_ids_for_positive_count("confirmation_required_execution_action_count")
    assert project_integrations_payload["preview_supported_execution_action_count"] == _sum_int_field("preview_supported_execution_action_count")
    assert project_integrations_payload["preview_supported_execution_action_refs"] == _collect_action_refs("preview_supported_execution_action_ids")
    assert project_integrations_payload["preview_supported_execution_action_family_count"] == sum(1 for item in family_list if item["preview_supported_execution_action_count"] > 0)
    assert project_integrations_payload["preview_supported_execution_action_family_ids"] == _family_ids_for_positive_count("preview_supported_execution_action_count")
    assert project_integrations_payload["context_available_execution_action_count"] == _sum_int_field("context_available_execution_action_count")
    assert project_integrations_payload["context_available_execution_action_refs"] == _collect_action_refs("context_available_execution_action_ids")
    assert project_integrations_payload["context_available_execution_action_family_count"] == sum(1 for item in family_list if item["context_available_execution_action_count"] > 0)
    assert project_integrations_payload["context_available_execution_action_family_ids"] == _family_ids_for_positive_count("context_available_execution_action_count")
    assert project_integrations_payload["provider_guidance_action_count"] == _sum_int_field("provider_guidance_action_count")
    assert project_integrations_payload["provider_guidance_action_refs"] == _collect_action_refs("provider_guidance_action_ids")
    assert project_integrations_payload["provider_guidance_action_family_count"] == sum(1 for item in family_list if item["provider_guidance_action_count"] > 0)
    assert project_integrations_payload["provider_guidance_action_family_ids"] == _family_ids_for_positive_count("provider_guidance_action_count")
    assert project_integrations_payload["commandful_execution_action_count"] == _sum_int_field("commandful_execution_action_count")
    assert project_integrations_payload["commandful_execution_action_refs"] == _collect_action_refs("commandful_execution_action_ids")
    assert project_integrations_payload["commandful_execution_action_family_count"] == sum(1 for item in family_list if item["commandful_execution_action_count"] > 0)
    assert project_integrations_payload["commandful_execution_action_family_ids"] == _family_ids_for_positive_count("commandful_execution_action_count")
    ...
    assert project_integrations_payload["blocked_execution_action_refs"] == _collect_action_refs("blocked_execution_action_ids")
    assert project_integrations_payload["blocked_local_execution_action_count"] == _sum_int_field("blocked_local_execution_action_count")
    assert project_integrations_payload["blocked_local_execution_action_refs"] == _collect_action_refs("blocked_local_execution_action_ids")
    assert project_integrations_payload["blocked_local_execution_action_family_count"] == sum(1 for item in family_list if item["blocked_local_execution_action_count"] > 0)
    assert project_integrations_payload["blocked_local_execution_action_family_ids"] == _family_ids_for_positive_count("blocked_local_execution_action_count")
    assert project_integrations_payload["blocked_guided_execution_action_count"] == _sum_int_field("blocked_guided_execution_action_count")
    assert project_integrations_payload["blocked_guided_execution_action_refs"] == _collect_action_refs("blocked_guided_execution_action_ids")
    assert project_integrations_payload["blocked_guided_execution_action_family_count"] == sum(1 for item in family_list if item["blocked_guided_execution_action_count"] > 0)
    assert project_integrations_payload["blocked_guided_execution_action_family_ids"] == _family_ids_for_positive_count("blocked_guided_execution_action_count")
    assert project_integrations_payload["blocked_provider_specific_execution_action_count"] == _sum_int_field("blocked_provider_specific_execution_action_count")
    assert project_integrations_payload["blocked_provider_specific_execution_action_refs"] == _collect_action_refs("blocked_provider_specific_execution_action_ids")
    assert project_integrations_payload["blocked_provider_specific_execution_action_family_count"] == sum(1 for item in family_list if item["blocked_provider_specific_execution_action_count"] > 0)
    assert project_integrations_payload["blocked_provider_specific_execution_action_family_ids"] == _family_ids_for_positive_count("blocked_provider_specific_execution_action_count")
    assert project_integrations_payload["blocked_guided_only_execution_action_count"] == _sum_int_field("blocked_guided_only_execution_action_count")
    assert project_integrations_payload["blocked_guided_only_execution_action_refs"] == _collect_action_refs("blocked_guided_only_execution_action_ids")
    assert project_integrations_payload["blocked_guided_only_execution_action_family_count"] == sum(1 for item in family_list if item["blocked_guided_only_execution_action_count"] > 0)
    assert project_integrations_payload["blocked_guided_only_execution_action_family_ids"] == _family_ids_for_positive_count("blocked_guided_only_execution_action_count")
    ...
    assert project_integrations_payload["safe_command_action_refs"] == _collect_action_refs("safe_command_action_ids")
    ...
    assert project_integrations_payload["available_provider_lane_action_refs"] == _collect_action_refs("available_provider_lane_action_ids")
    ...
    assert project_integrations_payload["verification_blocked_action_refs"] == _collect_action_refs("verification_blocked_action_ids")
    assert project_integrations_payload["verification_blocked_guided_action_count"] == _sum_int_field("verification_blocked_guided_action_count")
    assert project_integrations_payload["verification_blocked_guided_action_refs"] == _collect_action_refs("verification_blocked_guided_action_ids")
    assert project_integrations_payload["verification_blocked_guided_action_family_count"] == sum(1 for item in family_list if item["verification_blocked_guided_action_count"] > 0)
    assert project_integrations_payload["verification_blocked_guided_action_family_ids"] == _family_ids_for_positive_count("verification_blocked_guided_action_count")
    assert project_integrations_payload["verification_blocked_local_action_count"] == _sum_int_field("verification_blocked_local_action_count")
    assert project_integrations_payload["verification_blocked_local_action_refs"] == _collect_action_refs("verification_blocked_local_action_ids")
    assert project_integrations_payload["verification_blocked_local_action_family_count"] == sum(1 for item in family_list if item["verification_blocked_local_action_count"] > 0)
    assert project_integrations_payload["verification_blocked_local_action_family_ids"] == _family_ids_for_positive_count("verification_blocked_local_action_count")
    ...
    assert project_integrations_payload["multi_blocked_action_count"] == _sum_int_field("multi_blocked_action_count")
    assert project_integrations_payload["multi_blocked_action_refs"] == _collect_action_refs("multi_blocked_action_ids")
    assert project_integrations_payload["multi_blocked_action_family_count"] == sum(1 for item in family_list if item["multi_blocked_action_count"] > 0)
    assert project_integrations_payload["multi_blocked_action_family_ids"] == _family_ids_for_positive_count("multi_blocked_action_count")
    assert project_integrations_payload["defaulted_param_action_count"] == _sum_int_field("defaulted_param_action_count")
    assert project_integrations_payload["defaulted_param_action_refs"] == _collect_action_refs("defaulted_param_action_ids")
    assert project_integrations_payload["defaulted_param_action_family_count"] == sum(1 for item in family_list if item["defaulted_param_action_count"] > 0)
    assert project_integrations_payload["defaulted_param_action_family_ids"] == _family_ids_for_positive_count("defaulted_param_action_count")
    assert project_integrations_payload["missing_params_action_count"] == _sum_int_field("missing_params_action_count")
    assert project_integrations_payload["missing_params_action_refs"] == _collect_action_refs("missing_params_action_ids")
    assert project_integrations_payload["missing_params_action_family_count"] == sum(1 for item in family_list if item["missing_params_action_count"] > 0)
    assert project_integrations_payload["missing_params_action_family_ids"] == _family_ids_for_positive_count("missing_params_action_count")
    assert project_integrations_payload["missing_executable_action_count"] == _sum_int_field("missing_executable_action_count")
    assert project_integrations_payload["missing_executable_action_refs"] == _collect_action_refs("missing_executable_action_ids")
    assert project_integrations_payload["missing_executable_action_family_count"] == sum(1 for item in family_list if item["missing_executable_action_count"] > 0)
    assert project_integrations_payload["missing_executable_action_family_ids"] == _family_ids_for_positive_count("missing_executable_action_count")
    assert project_integrations_payload["no_local_command_action_count"] == _sum_int_field("no_local_command_action_count")
    assert project_integrations_payload["no_local_command_action_refs"] == _collect_action_refs("no_local_command_action_ids")
    assert project_integrations_payload["no_local_command_action_family_count"] == sum(1 for item in family_list if item["no_local_command_action_count"] > 0)
    assert project_integrations_payload["no_local_command_action_family_ids"] == _family_ids_for_positive_count("no_local_command_action_count")
    assert project_integrations_payload["provider_context_blocked_action_count"] == _sum_int_field("provider_context_blocked_action_count")
    assert project_integrations_payload["provider_context_blocked_action_refs"] == _collect_action_refs("provider_context_blocked_action_ids")
    assert project_integrations_payload["provider_context_blocked_action_family_count"] == sum(1 for item in family_list if item["provider_context_blocked_action_count"] > 0)
    assert project_integrations_payload["provider_context_blocked_action_family_ids"] == _family_ids_for_positive_count("provider_context_blocked_action_count")
    assert project_integrations_payload["provider_context_verified_action_count"] == _sum_int_field("provider_context_verified_action_count")
    assert project_integrations_payload["provider_context_verified_action_refs"] == _collect_action_refs("provider_context_verified_action_ids")
    assert project_integrations_payload["provider_context_verified_action_family_count"] == sum(1 for item in family_list if item["provider_context_verified_action_count"] > 0)
    assert project_integrations_payload["provider_context_verified_action_family_ids"] == _family_ids_for_positive_count("provider_context_verified_action_count")
    assert project_integrations_payload["provider_context_inferred_action_count"] == _sum_int_field("provider_context_inferred_action_count")
    assert project_integrations_payload["provider_context_inferred_action_refs"] == _collect_action_refs("provider_context_inferred_action_ids")
    assert project_integrations_payload["provider_context_inferred_action_family_count"] == sum(1 for item in family_list if item["provider_context_inferred_action_count"] > 0)
    assert project_integrations_payload["provider_context_inferred_action_family_ids"] == _family_ids_for_positive_count("provider_context_inferred_action_count")
    assert project_integrations_payload["provider_context_missing_action_count"] == _sum_int_field("provider_context_missing_action_count")
    assert project_integrations_payload["provider_context_missing_action_refs"] == _collect_action_refs("provider_context_missing_action_ids")
    assert project_integrations_payload["provider_context_missing_action_family_count"] == sum(1 for item in family_list if item["provider_context_missing_action_count"] > 0)
    assert project_integrations_payload["provider_context_missing_action_family_ids"] == _family_ids_for_positive_count("provider_context_missing_action_count")
    assert project_integrations_payload["commandless_execution_action_count"] == _sum_int_field("commandless_execution_action_count")
    assert project_integrations_payload["commandless_execution_action_refs"] == _collect_action_refs("commandless_execution_action_ids")
    assert project_integrations_payload["commandless_execution_action_family_count"] == sum(1 for item in family_list if item["commandless_execution_action_count"] > 0)
    assert project_integrations_payload["commandless_execution_action_family_ids"] == _family_ids_for_positive_count("commandless_execution_action_count")
    ...
    assert project_integrations_payload["action_status_action_refs"] == _collect_grouped_action_refs("action_status_action_ids")
    assert project_integrations_payload["action_status_family_ids"] == _group_count_map_family_ids("action_status_counts")
    assert project_integrations_payload["available_action_statuses"] == sorted(project_integrations_payload["available_action_status_counts"])
    assert project_integrations_payload["available_action_status_counts"] == _sum_count_maps("available_action_status_counts")
    assert project_integrations_payload["available_action_status_group_count"] == len(project_integrations_payload["available_action_status_counts"])
    assert project_integrations_payload["available_action_status_action_refs"] == _collect_grouped_action_refs("available_action_status_action_ids")
    assert project_integrations_payload["available_action_status_family_ids"] == _group_count_map_family_ids("available_action_status_counts")
    assert project_integrations_payload["blocked_action_statuses"] == sorted(project_integrations_payload["blocked_action_status_counts"])
    assert project_integrations_payload["blocked_action_status_counts"] == _sum_count_maps("blocked_action_status_counts")
    assert project_integrations_payload["blocked_action_status_group_count"] == len(project_integrations_payload["blocked_action_status_counts"])
    assert project_integrations_payload["blocked_action_status_action_refs"] == _collect_grouped_action_refs("blocked_action_status_action_ids")
    assert project_integrations_payload["blocked_action_status_family_ids"] == _group_count_map_family_ids("blocked_action_status_counts")
    assert project_integrations_payload["execution_modes"] == sorted(project_integrations_payload["execution_mode_counts"])
    ...
    assert project_integrations_payload["execution_mode_action_refs"] == _collect_grouped_action_refs("execution_mode_action_ids")
    assert project_integrations_payload["execution_mode_family_ids"] == _group_count_map_family_ids("execution_mode_counts")
    assert project_integrations_payload["available_execution_modes"] == sorted(project_integrations_payload["available_execution_mode_counts"])
    assert project_integrations_payload["available_execution_mode_counts"] == _sum_count_maps("available_execution_mode_counts")
    assert project_integrations_payload["available_execution_mode_group_count"] == len(project_integrations_payload["available_execution_mode_counts"])
    assert project_integrations_payload["available_execution_mode_action_refs"] == _collect_grouped_action_refs("available_execution_mode_action_ids")
    assert project_integrations_payload["available_execution_mode_family_ids"] == _group_count_map_family_ids("available_execution_mode_counts")
    assert project_integrations_payload["blocked_execution_modes"] == sorted(project_integrations_payload["blocked_execution_mode_counts"])
    assert project_integrations_payload["blocked_execution_mode_counts"] == _sum_count_maps("blocked_execution_mode_counts")
    assert project_integrations_payload["blocked_execution_mode_group_count"] == len(project_integrations_payload["blocked_execution_mode_counts"])
    assert project_integrations_payload["blocked_execution_mode_action_refs"] == _collect_grouped_action_refs("blocked_execution_mode_action_ids")
    assert project_integrations_payload["blocked_execution_mode_family_ids"] == _group_count_map_family_ids("blocked_execution_mode_counts")
    assert project_integrations_payload["provider_support_modes"] == sorted(project_integrations_payload["provider_support_mode_counts"])
    ...
    assert project_integrations_payload["provider_support_mode_action_refs"] == _collect_grouped_action_refs("provider_support_mode_action_ids")
    assert project_integrations_payload["provider_support_mode_family_ids"] == _group_count_map_family_ids("provider_support_mode_counts")
    assert project_integrations_payload["available_provider_support_modes"] == sorted(project_integrations_payload["available_provider_support_mode_counts"])
    assert project_integrations_payload["available_provider_support_mode_counts"] == _sum_count_maps("available_provider_support_mode_counts")
    assert project_integrations_payload["available_provider_support_mode_group_count"] == len(project_integrations_payload["available_provider_support_mode_counts"])
    assert project_integrations_payload["available_provider_support_mode_action_refs"] == _collect_grouped_action_refs("available_provider_support_mode_action_ids")
    assert project_integrations_payload["available_provider_support_mode_family_ids"] == _group_count_map_family_ids("available_provider_support_mode_counts")
    assert project_integrations_payload["blocked_provider_support_modes"] == sorted(project_integrations_payload["blocked_provider_support_mode_counts"])
    assert project_integrations_payload["blocked_provider_support_mode_counts"] == _sum_count_maps("blocked_provider_support_mode_counts")
    assert project_integrations_payload["blocked_provider_support_mode_group_count"] == len(project_integrations_payload["blocked_provider_support_mode_counts"])
    assert project_integrations_payload["blocked_provider_support_mode_action_refs"] == _collect_grouped_action_refs("blocked_provider_support_mode_action_ids")
    assert project_integrations_payload["blocked_provider_support_mode_family_ids"] == _group_count_map_family_ids("blocked_provider_support_mode_counts")
    assert project_integrations_payload["action_provider_context_statuses"] == sorted(project_integrations_payload["action_provider_context_status_counts"])
    ...
    assert project_integrations_payload["action_provider_context_status_action_refs"] == _collect_grouped_action_refs("provider_context_status_action_ids")
    assert project_integrations_payload["action_provider_context_status_family_ids"] == _group_count_map_family_ids("provider_context_status_counts")
    assert project_integrations_payload["available_action_provider_context_statuses"] == sorted(project_integrations_payload["available_action_provider_context_status_counts"])
    assert project_integrations_payload["available_action_provider_context_status_counts"] == _sum_count_maps("available_provider_context_status_counts")
    assert project_integrations_payload["available_action_provider_context_status_group_count"] == len(project_integrations_payload["available_action_provider_context_status_counts"])
    assert project_integrations_payload["available_action_provider_context_status_action_refs"] == _collect_grouped_action_refs("available_provider_context_status_action_ids")
    assert project_integrations_payload["available_action_provider_context_status_family_ids"] == _group_count_map_family_ids("available_provider_context_status_counts")
    assert project_integrations_payload["blocked_action_provider_context_statuses"] == sorted(project_integrations_payload["blocked_action_provider_context_status_counts"])
    assert project_integrations_payload["blocked_action_provider_context_status_counts"] == _sum_count_maps("blocked_provider_context_status_counts")
    assert project_integrations_payload["blocked_action_provider_context_status_group_count"] == len(project_integrations_payload["blocked_action_provider_context_status_counts"])
    assert project_integrations_payload["blocked_action_provider_context_status_action_refs"] == _collect_grouped_action_refs("blocked_provider_context_status_action_ids")
    assert project_integrations_payload["blocked_action_provider_context_status_family_ids"] == _group_count_map_family_ids("blocked_provider_context_status_counts")
    assert project_integrations_payload["available_provider_context_statuses"] == sorted(project_integrations_payload["available_provider_context_status_counts"])
    assert project_integrations_payload["available_provider_context_status_counts"] == _sum_count_maps("available_provider_context_status_counts")
    assert project_integrations_payload["available_provider_context_status_group_count"] == len(project_integrations_payload["available_provider_context_status_counts"])
    assert project_integrations_payload["available_provider_context_status_action_refs"] == _collect_grouped_action_refs("available_provider_context_status_action_ids")
    assert project_integrations_payload["available_provider_context_status_family_ids"] == _group_count_map_family_ids("available_provider_context_status_counts")
    assert project_integrations_payload["blocked_provider_context_statuses"] == sorted(project_integrations_payload["blocked_provider_context_status_counts"])
    assert project_integrations_payload["blocked_provider_context_status_counts"] == _sum_count_maps("blocked_provider_context_status_counts")
    assert project_integrations_payload["blocked_provider_context_status_group_count"] == len(project_integrations_payload["blocked_provider_context_status_counts"])
    assert project_integrations_payload["blocked_provider_context_status_action_refs"] == _collect_grouped_action_refs("blocked_provider_context_status_action_ids")
    assert project_integrations_payload["blocked_provider_context_status_family_ids"] == _group_count_map_family_ids("blocked_provider_context_status_counts")
    assert project_integrations_payload["verification_scopes"] == sorted(project_integrations_payload["verification_scope_counts"])
    ...
    assert project_integrations_payload["verification_scope_action_refs"] == _collect_grouped_action_refs("verification_scope_action_ids")
    assert project_integrations_payload["verification_scope_family_ids"] == _group_count_map_family_ids("verification_scope_counts")
    assert project_integrations_payload["available_verification_scopes"] == sorted(project_integrations_payload["available_verification_scope_counts"])
    assert project_integrations_payload["available_verification_scope_counts"] == _sum_count_maps("available_verification_scope_counts")
    assert project_integrations_payload["available_verification_scope_group_count"] == len(project_integrations_payload["available_verification_scope_counts"])
    assert project_integrations_payload["available_verification_scope_action_refs"] == _collect_grouped_action_refs("available_verification_scope_action_ids")
    assert project_integrations_payload["available_verification_scope_family_ids"] == _group_count_map_family_ids("available_verification_scope_counts")
    assert project_integrations_payload["blocked_verification_scopes"] == sorted(project_integrations_payload["blocked_verification_scope_counts"])
    assert project_integrations_payload["blocked_verification_scope_counts"] == _sum_count_maps("blocked_verification_scope_counts")
    assert project_integrations_payload["blocked_verification_scope_group_count"] == len(project_integrations_payload["blocked_verification_scope_counts"])
    assert project_integrations_payload["blocked_verification_scope_action_refs"] == _collect_grouped_action_refs("blocked_verification_scope_action_ids")
    assert project_integrations_payload["blocked_verification_scope_family_ids"] == _group_count_map_family_ids("blocked_verification_scope_counts")
    assert project_integrations_payload["safe_command_reasons"] == sorted(project_integrations_payload["safe_command_reason_counts"])
    ...
    assert project_integrations_payload["safe_command_reason_action_refs"] == _collect_grouped_action_refs("safe_command_reason_action_ids")
    assert project_integrations_payload["safe_command_reason_family_ids"] == _group_count_map_family_ids("safe_command_reason_counts")
    assert project_integrations_payload["available_safe_command_reasons"] == sorted(project_integrations_payload["available_safe_command_reason_counts"])
    assert project_integrations_payload["available_safe_command_reason_counts"] == _sum_count_maps("available_safe_command_reason_counts")
    assert project_integrations_payload["available_safe_command_reason_group_count"] == len(project_integrations_payload["available_safe_command_reason_counts"])
    assert project_integrations_payload["available_safe_command_reason_action_refs"] == _collect_grouped_action_refs("available_safe_command_reason_action_ids")
    assert project_integrations_payload["available_safe_command_reason_family_ids"] == _group_count_map_family_ids("available_safe_command_reason_counts")
    assert project_integrations_payload["blocked_safe_command_reasons"] == sorted(project_integrations_payload["blocked_safe_command_reason_counts"])
    assert project_integrations_payload["blocked_safe_command_reason_counts"] == _sum_count_maps("blocked_safe_command_reason_counts")
    assert project_integrations_payload["blocked_safe_command_reason_group_count"] == len(project_integrations_payload["blocked_safe_command_reason_counts"])
    assert project_integrations_payload["blocked_safe_command_reason_action_refs"] == _collect_grouped_action_refs("blocked_safe_command_reason_action_ids")
    assert project_integrations_payload["blocked_safe_command_reason_family_ids"] == _group_count_map_family_ids("blocked_safe_command_reason_counts")
    assert project_integrations_payload["context_requirement_reasons"] == sorted(project_integrations_payload["context_requirement_reason_counts"])
    ...
    assert project_integrations_payload["context_requirement_reason_action_refs"] == _collect_grouped_action_refs("context_requirement_reason_action_ids")
    assert project_integrations_payload["context_requirement_reason_family_ids"] == _group_count_map_family_ids("context_requirement_reason_counts")
    assert project_integrations_payload["available_context_requirement_reasons"] == sorted(project_integrations_payload["available_context_requirement_reason_counts"])
    assert project_integrations_payload["available_context_requirement_reason_counts"] == _sum_count_maps("available_context_requirement_reason_counts")
    assert project_integrations_payload["available_context_requirement_reason_group_count"] == len(project_integrations_payload["available_context_requirement_reason_counts"])
    assert project_integrations_payload["available_context_requirement_reason_action_refs"] == _collect_grouped_action_refs("available_context_requirement_reason_action_ids")
    assert project_integrations_payload["available_context_requirement_reason_family_ids"] == _group_count_map_family_ids("available_context_requirement_reason_counts")
    assert project_integrations_payload["blocked_context_requirement_reasons"] == sorted(project_integrations_payload["blocked_context_requirement_reason_counts"])
    assert project_integrations_payload["blocked_context_requirement_reason_counts"] == _sum_count_maps("blocked_context_requirement_reason_counts")
    assert project_integrations_payload["blocked_context_requirement_reason_group_count"] == len(project_integrations_payload["blocked_context_requirement_reason_counts"])
    assert project_integrations_payload["blocked_context_requirement_reason_action_refs"] == _collect_grouped_action_refs("blocked_context_requirement_reason_action_ids")
    assert project_integrations_payload["blocked_context_requirement_reason_family_ids"] == _group_count_map_family_ids("blocked_context_requirement_reason_counts")
    assert project_integrations_payload["action_providers"] == sorted(project_integrations_payload["action_provider_counts"])
    assert project_integrations_payload["action_provider_counts"] == _sum_count_maps("action_provider_counts")
    assert project_integrations_payload["action_provider_group_count"] == len(project_integrations_payload["action_provider_counts"])
    assert project_integrations_payload["action_provider_action_refs"] == _collect_grouped_action_refs("action_provider_action_ids")
    assert project_integrations_payload["action_provider_family_ids"] == _group_count_map_family_ids("action_provider_counts")
    assert project_integrations_payload["available_action_providers"] == sorted(project_integrations_payload["available_action_provider_counts"])
    assert project_integrations_payload["available_action_provider_counts"] == _sum_count_maps("available_action_provider_counts")
    assert project_integrations_payload["available_action_provider_group_count"] == len(project_integrations_payload["available_action_provider_counts"])
    assert project_integrations_payload["available_action_provider_action_refs"] == _collect_grouped_action_refs("available_action_provider_action_ids")
    assert project_integrations_payload["available_action_provider_family_ids"] == _group_count_map_family_ids("available_action_provider_counts")
    assert project_integrations_payload["blocked_action_providers"] == sorted(project_integrations_payload["blocked_action_provider_counts"])
    assert project_integrations_payload["blocked_action_provider_counts"] == _sum_count_maps("blocked_action_provider_counts")
    assert project_integrations_payload["blocked_action_provider_group_count"] == len(project_integrations_payload["blocked_action_provider_counts"])
    assert project_integrations_payload["blocked_action_provider_action_refs"] == _collect_grouped_action_refs("blocked_action_provider_action_ids")
    assert project_integrations_payload["blocked_action_provider_family_ids"] == _group_count_map_family_ids("blocked_action_provider_counts")
    assert project_integrations_payload["executable_names"] == sorted(project_integrations_payload["executable_name_counts"])
    assert project_integrations_payload["executable_name_counts"] == _sum_count_maps("executable_name_counts")
    assert project_integrations_payload["executable_name_group_count"] == len(project_integrations_payload["executable_name_counts"])
    assert project_integrations_payload["executable_name_action_refs"] == _collect_grouped_action_refs("executable_name_action_ids")
    assert project_integrations_payload["executable_name_family_ids"] == _group_count_map_family_ids("executable_name_counts")
    assert project_integrations_payload["available_executable_names"] == sorted(project_integrations_payload["available_executable_name_counts"])
    assert project_integrations_payload["available_executable_name_counts"] == _sum_count_maps("available_executable_name_counts")
    assert project_integrations_payload["available_executable_name_group_count"] == len(project_integrations_payload["available_executable_name_counts"])
    assert project_integrations_payload["available_executable_name_action_refs"] == _collect_grouped_action_refs("available_executable_name_action_ids")
    assert project_integrations_payload["available_executable_name_family_ids"] == _group_count_map_family_ids("available_executable_name_counts")
    assert project_integrations_payload["blocked_executable_names"] == sorted(project_integrations_payload["blocked_executable_name_counts"])
    assert project_integrations_payload["blocked_executable_name_counts"] == _sum_count_maps("blocked_executable_name_counts")
    assert project_integrations_payload["blocked_executable_name_group_count"] == len(project_integrations_payload["blocked_executable_name_counts"])
    assert project_integrations_payload["blocked_executable_name_action_refs"] == _collect_grouped_action_refs("blocked_executable_name_action_ids")
    assert project_integrations_payload["blocked_executable_name_family_ids"] == _group_count_map_family_ids("blocked_executable_name_counts")
    assert project_integrations_payload["execution_block_reasons"] == sorted(project_integrations_payload["execution_block_reason_counts"])
    ...
    assert project_integrations_payload["execution_block_reason_action_refs"] == _collect_grouped_action_refs("execution_block_reason_action_ids")
    assert project_integrations_payload["execution_block_reason_family_ids"] == _group_count_map_family_ids("execution_block_reason_counts")
    assert project_integrations_payload["blocking_reasons"] == sorted(project_integrations_payload["blocking_reason_counts"])
    ...
    assert project_integrations_payload["blocking_reason_action_refs"] == _collect_grouped_action_refs("blocking_reason_action_ids")
    assert project_integrations_payload["blocking_reason_family_ids"] == _group_count_map_family_ids("blocking_reason_counts")
    ...
    assert project_integrations_payload["permission_policies"] == sorted(project_integrations_payload["permission_policy_counts"])
    assert project_integrations_payload["permission_policy_counts"] == _sum_count_maps("permission_policy_counts")
    ...
    assert project_integrations_payload["available_permission_policy_counts"] == _sum_count_maps("available_permission_policy_counts")
    ...
    assert project_integrations_payload["blocked_permission_policy_counts"] == _sum_count_maps("blocked_permission_policy_counts")
    ...
    assert project_integrations_payload["risk_level_counts"] == _sum_count_maps("risk_level_counts")
    ...
    assert project_integrations_payload["available_risk_level_counts"] == _sum_count_maps("available_risk_level_counts")
    ...
    assert project_integrations_payload["blocked_risk_level_counts"] == _sum_count_maps("blocked_risk_level_counts")
    ...
    assert project_integrations_payload["execution_permission_policies"] == sorted(project_integrations_payload["execution_permission_policy_counts"])
    assert project_integrations_payload["execution_permission_policy_counts"] == _sum_count_maps("execution_permission_policy_counts")
    ...
    assert project_integrations_payload["available_execution_permission_policy_counts"] == _sum_count_maps("available_execution_permission_policy_counts")
    ...
    assert project_integrations_payload["blocked_execution_permission_policy_counts"] == _sum_count_maps("blocked_execution_permission_policy_counts")
    ...
    assert project_integrations_payload["execution_risk_level_counts"] == _sum_count_maps("execution_risk_level_counts")
    ...
    assert project_integrations_payload["available_execution_risk_level_counts"] == _sum_count_maps("available_execution_risk_level_counts")
    ...
    assert project_integrations_payload["blocked_execution_risk_level_counts"] == _sum_count_maps("blocked_execution_risk_level_counts")
...
def test_project_integrations_surface_execution_block_reason_inventories(monkeypatch, tmp_path) -> None:
    ...
    assert ci_status["execution_block_reason_group_count"] == 1
    assert ci_status["blocking_reason_group_count"] == 2
    assert ci_status["blocked_action_count"] >= 3
    assert ci_status["connection_status"] == "disconnected"
    ...
    assert ci_status["health"]["execution_block_reason_group_count"] == 1
    assert ci_status["health"]["blocking_reason_group_count"] == 2
    assert ci_status["health"]["blocked_action_count"] >= 3
    assert ci_status["health"]["connection_status"] == "disconnected"
    ...
    assert package_status["blocked_action_count"] >= 1
    ...
    assert package_status["execution_block_reason_group_count"] == 1
    assert package_status["blocking_reason_group_count"] == 1
    ...
    assert package_status["health"]["execution_block_reason_group_count"] == 1
    assert package_status["health"]["blocking_reason_group_count"] == 1
...
def test_project_integrations_surface_permission_and_risk_inventories(monkeypatch, tmp_path) -> None:
    ...
    assert ci_status["permission_policy_group_count"] == 2
    assert ci_status["available_permission_policy_group_count"] == 2
    assert ci_status["blocked_permission_policy_group_count"] == 2
    ...
    assert ci_status["risk_level_group_count"] == 2
    assert ci_status["available_risk_level_group_count"] == 2
    assert ci_status["blocked_risk_level_group_count"] == 2
    ...
    assert ci_status["execution_permission_policy_group_count"] == 2
    ...
    assert ci_status["available_execution_permission_policy_group_count"] == 1
    ...
    assert ci_status["blocked_execution_permission_policy_group_count"] == 2
    ...
    assert ci_status["execution_risk_level_group_count"] == 2
    ...
    assert ci_status["available_execution_risk_level_group_count"] == 1
    ...
    assert ci_status["blocked_execution_risk_level_group_count"] == 2
    ...
    assert package_status["permission_policy_group_count"] == 2
    assert package_status["available_permission_policy_group_count"] == 2
    assert package_status["blocked_permission_policy_group_count"] == 1
    ...
    assert package_status["risk_level_group_count"] == 3
    assert package_status["available_risk_level_group_count"] == 2
    assert package_status["blocked_risk_level_group_count"] == 1
    ...
    assert package_status["execution_permission_policy_group_count"] == 2
    ...
    assert package_status["available_execution_permission_policy_group_count"] == 1
    ...
    assert package_status["blocked_execution_permission_policy_group_count"] == 1
    ...
    assert package_status["execution_risk_level_group_count"] == 2
    ...
    assert package_status["available_execution_risk_level_group_count"] == 1
    ...
    assert package_status["blocked_execution_risk_level_group_count"] == 1
