...
def _require_widget_instance_scope(db: Session, instance_id: int, project_id: int | None) -> Any:
    instance = service._widget_instance_or_error(db, instance_id)
    if instance.project_id is None:
        return instance
    if project_id is None:
        raise HTTPException(status_code=404, detail="Widget instance not found")
    _require_project_scope("Widget instance", instance.project_id, project_id)
    return instance
...
def patch_widget_instance(instance_id: int, payload: WidgetInstanceUpdate, db: Session = Depends(get_db)) -> WidgetInstanceRead:
    try:
        _require_widget_instance_scope(db, instance_id, payload.project_id)
...
def delete_widget_instance(instance_id: int, project_id: int | None = Query(default=None), db: Session = Depends(get_db)) -> None:
    try:
        _require_widget_instance_scope(db, instance_id, project_id)
