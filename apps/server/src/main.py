...
def _require_project_agent(db: Session, agent_id: int, project_id: int, *, resource_name: str = "Agent") -> Agent:
    agent = _get_agent_or_404(db, agent_id)
    _require_project_scope(resource_name, agent.project_id, project_id)
    return agent


def _require_project_task(db: Session, task_id: int, project_id: int, *, resource_name: str = "Task") -> Task:
    task = _get_task_or_404(db, task_id)
    _require_project_scope(resource_name, task.project_id, project_id)
    return task


def _require_project_run(db: Session, run_id: int, project_id: int, *, resource_name: str = "Run") -> AgentRun:
    run = db.get(AgentRun, run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
    agent = db.get(Agent, run.agent_id)
    actual_project_id = agent.project_id if agent is not None else None
    if actual_project_id is None and run.task_id is not None:
        task = db.get(Task, run.task_id)
        actual_project_id = task.project_id if task is not None else None
    _require_project_scope(resource_name, actual_project_id, project_id)
    return run
...
async def start_agent(agent_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> AgentActionResponse:
    agent = _require_project_agent(db, agent_id, project_id)
    project = _get_project_or_404(db, project_id)
...
async def stop_agent(agent_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> AgentActionResponse:
    agent = _require_project_agent(db, agent_id, project_id)
...
async def pause_agent(agent_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> AgentActionResponse:
    agent = _require_project_agent(db, agent_id, project_id)
...
def get_agent_logs(agent_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> LogRead:
    agent = _require_project_agent(db, agent_id, project_id)
...
async def start_task(task_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> AgentActionResponse:
    task = _require_project_task(db, task_id, project_id)
    project = _get_project_or_404(db, project_id)
...
async def complete_task(task_id: int, project_id: int = Query(...), db: Session = Depends(get_db)) -> AgentActionResponse:
    task = _require_project_task(db, task_id, project_id)
...
async def submit_run_report(
    ...
    run = _require_project_run(db, run_id, project_id)
    try:
