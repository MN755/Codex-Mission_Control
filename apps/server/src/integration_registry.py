...
def _count_by_key(items: list[dict[str, Any]], key: str) -> dict[str, int]:
    return {
        str(name): count
        for name, count in Counter(str(item[key]) for item in items if item.get(key) is not None).items()
        if count > 0
    }


def _action_ids_by_key(items: list[dict[str, Any]], key: str) -> dict[str, list[str]]:
    grouped: dict[str, list[str]] = {}
    for item in items:
        if item.get(key) is None:
            continue
        grouped.setdefault(str(item[key]), []).append(str(item["action_id"]))
...
def build_project_integration_status(
    *,
    ...
    for family in FAMILIES:
        ...
        permission_policy_counts = _count_by_key(available_actions, "permission_policy")
        available_permission_policy_counts = _count_by_key(actionable_actions, "permission_policy")
        blocked_permission_policy_counts = _count_by_key(blocked_actions, "permission_policy")
        ...
        executable_name_counts = _count_by_key(available_actions, "executable_name")
        executable_name_action_ids = _action_ids_by_key(available_actions, "executable_name")
        executable_name_group_count = _group_count(executable_name_counts)
        ...
        statuses.append(
            {
                ...
                "provider_count": len(displayed_providers),
                "resolved_provider": resolved_provider,
                "provider_candidates": provider_candidates,
                "provider_candidate_count": len(provider_candidates),
                "resolved_cli_candidates": provider_cli_candidates,
                "resolved_cli_candidate_count": len(provider_cli_candidates),
                "provider_signal_breakdown": provider_signal_breakdown,
                "resolved_provider_evidence": dict(provider_signal_breakdown.get(resolved_provider) or {}),
                "cli_only_candidates_suppressed": cli_only_candidates_suppressed,
                "cli_only_candidates_suppressed_count": len(cli_only_candidates_suppressed),
                "provider_resolution_state": provider_resolution_state,
                "provider_context_verified": provider_context_verified,
                "provider_context_source": provider_context_source,
                "provider_context_status": _provider_context_status(
                ...
                "cli_detected_count": len(installed_clis),
                "resolved_cli_detected": installed_provider_clis,
                "resolved_cli_detected_count": len(installed_provider_clis),
                "workspace_config_files": detected_files,
                "workspace_config_file_count": len(detected_files),
                "workspace_token_hits": token_hits,
                "workspace_token_hit_count": len(token_hits),
